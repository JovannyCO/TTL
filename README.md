...
Cómo y para qué se ajusta el **TTL** (IPv4) y el **Hop Limit** (IPv6) de un computador que navega con los datos del celular —zona Wi-Fi, *hotspot* o *tethering*— en Colombia. Cubre Windows 10 y 11, Linux y WSL2.

> **En corto:** el celular sale a internet con TTL 64 y le resta 1 a todo lo que reenvía. Si el computador sale con 65, a la red del operador le llega 64 desde los dos.
...
El ajuste hace que el tráfico del computador llegue a la red del operador con el mismo TTL que el del celular, así que ese valor ya no lo delata como uso compartido. Es útil para trabajar desde el portátil con los datos del celular: teletrabajo, viajes o lugares sin conexión fija.

Antes de hacerlo, lee [qué no hace](#qué-no-hace) y el [marco en Colombia](#marco-en-colombia).

## Cómo funciona

Cada paquete IP lleva un contador de saltos: el **TTL** en IPv4 ([RFC 791](https://www.rfc-editor.org/rfc/rfc791)) y el **Hop Limit** en IPv6 ([RFC 8200](https://www.rfc-editor.org/rfc/rfc8200)). El sistema operativo le pone un valor inicial y cada router que reenvía el paquete le resta 1. En la zona Wi-Fi, el celular es un router con NAT: a todo lo que llega del computador le resta 1 antes de mandarlo a la red del operador.
